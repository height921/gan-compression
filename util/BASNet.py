...
class BASNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.sigmoid(d1), F.sigmoid(d2), F.sigmoid(d3), F.sigmoid(d4), F.sigmoid(d5), F.sigmoid(
            d6), F.sigmoid(db)
...
if __name__ == '__main__':
    ...
    model = BASNet(3,1)
    y = model(x)
    t = make_dot(y,params=dict(list(BASNet(3,1).named_parameters()) + [('x', x)]))
    # t.format = "png"
    # 指定文件生成的文件夹
    t.directory = "ttttt"
    t.render('modelviz', view=False)

    # import hiddenlayer as h
    #
    # vis_graph = h.build_graph(BASNet(3,1), torch.zeros([1, 3, 512, 512]))  # 获取绘制图像的对象
